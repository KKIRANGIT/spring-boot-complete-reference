...
/**
 * CQRS write-side service for all account mutations.
 *
 * <p>Plain English: this is the only place allowed to create accounts, debit balances, credit
 * balances, and change account status.
 *
 * <p>Design decision: write methods always hit MySQL and never serve from cache, because commands
 * must use the freshest balance and status state available.
 *
 * <p>Interview question answered: "How do you design the write side of CQRS for banking balance
 * changes with retries and audit logs?"
 */
@Service
public class AccountCommandService {
  ...
  /**
   * Creates a new account and its initial audit record.
   */
  @Transactional
  ...
  /**
   * Debits money from an account with optimistic-lock retries.
   */
  @Transactional
  @Retryable(
  ...
  /**
   * Credits money to an account with optimistic-lock retries.
   */
  @Transactional
  @Retryable(
      retryFor = ObjectOptimisticLockingFailureException.class,
  ...
  /**
   * Changes account status and records the action for audit purposes.
   */
  @Transactional
